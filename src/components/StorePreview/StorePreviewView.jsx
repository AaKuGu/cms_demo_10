...
import { getMapEmbedSrc } from "./storePreview.utils";
...
const StorePreviewView = ({ shop, categories, products, socials, contact }) => {
    ...
    return (
        <div className="flex h-screen scroll-smooth flex-col bg-white">
            <FloatingSocialLinks links={visibleSocialLinks} />
            <FloatingQuickContact phone={shop.phone} whatsappMessage={whatsappMessage} />

            <StoreHeader shop={shop} whatsappMessage={whatsappMessage} onNavClick={scrollToSection} />
            ...
            {/* Scrollable body — products, then contact, then socials footer.
                ...
                so the contact block can never sit on top of / block the products. */}
            <div className="flex-1 overflow-y-auto bg-white">
                <div id="products" className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-10 lg:px-8">
                    <ProductGrid
                        hasAnyCategories={categoriesWithProducts.length > 0}
                ...
                {hasContactContent && (
                    <section id="contact">
                        <ContactSection contact={visibleContact} mapEmbedSrc={mapEmbedSrc} />
                    </section>
                )}
            </div>
        </div>
